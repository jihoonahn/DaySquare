...
public final class ShakeServiceImpl: ShakeService {
    ...
    private var continuations: [UUID: AsyncStream<ShakeEntity>.Continuation] = [:]
    private var pendingMotionData: [UUID: [ShakeEntity]] = [:]
    private var waitingTasks: [UUID: [CheckedContinuation<ShakeEntity, Error>]] = [:]
    ...
    public func startMonitoring(for alarmId: UUID, executionId: UUID, requiredCount: Int) async throws {
        guard motionManager.isAccelerometerAvailable else {
            throw ShakeServiceError.accelerometerNotAvailable
        }
        ...
        if motionManager.isDeviceMotionAvailable {
            ...
            motionManager.startDeviceMotionUpdates(using: .xMagneticNorthZVertical, to: queue) { [weak self, alarmIdKey, requiredCount] motion, error in
                ...
                if let error = error {
                    print("❌ [ShakeService] 흔들기 에러: \(error)")
                    return
                }
            ...
            motionManager.startAccelerometerUpdates(to: queue) { [weak self, alarmIdKey, requiredCount] data, error in
                ...
                if sampleCount <= 10 || sampleCount % 50 == 0 {
                    ...
                    print("📊 [ShakeService] 가속도계 샘플 #\(sampleCount): accel=\(String(format: "%.2f", accel)), accel=(\(String(format: "%.2f", data.acceleration.x)), \(String(format: "%.2f", data.acceleration.y)), \(String(format: "%.2f", data.acceleration.z)))")
                }
            ...
            print("✅ [ShakeService] Accelerometer 업데이트 시작: \(alarmIdKey)")
        }
    }
    ...
    private func processAlarmMotion(
        ...
        print("✅ [ShakeService] 흔들기 감지: \(alarmIdKey) - 카운트: \(currentCount + 1)/\(requiredCount), delta=\(String(format: "%.2f", delta)), change=\(String(format: "%.2f", change))")
        ...
        guard let executionId = executionId else {
            print("⚠️ [ShakeService] ShakeDetected: executionId를 찾을 수 없음 (alarmId=\(alarmIdKey))")
            return
        }
        
        NotificationCenter.default.post(
            name: NSNotification.Name("ShakeDetected"),
            object: nil,
            userInfo: [
                "alarmId": alarmIdKey,
    ...
    public func getShakeCount(for alarmId: UUID) -> Int {
        let alarmIdKey = alarmId.uuidString
    ...
    private func createShakeEntity(
        executionId: UUID,
        accelX: Double,
        accelY: Double,
        accelZ: Double,
